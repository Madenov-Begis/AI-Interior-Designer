...
function imagePart(image: ProviderImage): Part {
  return { inlineData: { data: image.data.toString("base64"), mimeType: image.mimeType } };
}
...
function buildParts(input: ProviderInput): Part[] {
  ...
  if (input.references.length > 0) {
    parts.push({ text: "The following images are style, material, furniture, and object references. Use their relevant design ideas without copying their room geometry or camera viewpoint." });
    input.references.forEach((reference, index) => {
      parts.push({ text: `Reference image ${index + 1}:` }, imagePart(reference));
    });
  }
