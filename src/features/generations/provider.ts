...
import { VertexGeminiImageProvider } from "@/features/generations/vertex-provider";
...
export class FakeImageGenerationProvider implements ImageGenerationProvider {
  async generate(input: ProviderInput): Promise<ProviderOutput> {
    ...
    const image = await sharp(input.source.data).rotate().toColorspace("srgb").composite([{ input: overlay, blend: "over" }]).webp({ quality: 92 }).toBuffer();
    return { image, mimeType: "image/webp", width: metadata.width, height: metadata.height, providerRequestId: `fake-${crypto.randomUUID()}` };
  }
}

export function getImageGenerationProvider(provider: AiProvider, modelId: string, timeoutSeconds: number): ImageGenerationProvider {
  if (provider === "FAKE") return new FakeImageGenerationProvider();
  if (provider === "VERTEX_AI") return new VertexGeminiImageProvider(modelId, timeoutSeconds);
