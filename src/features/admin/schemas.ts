...
export const planSchema = z.object({
  ...
  watermarkRequired: z.boolean(), priorityProcessing: z.boolean(), active: z.boolean(), sortOrder: z.number().int(),
});
export const updatePlanSchema = planSchema.partial().refine((value) => Object.keys(value).length > 0, "Нет изменений");

export const creditAdjustmentSchema = z.object({
  amount: z.number().int().min(-100_000).max(100_000).refine((value) => value !== 0, "Сумма корректировки не может быть нулевой"),
