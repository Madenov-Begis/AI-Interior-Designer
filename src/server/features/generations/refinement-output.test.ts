...
import { constrainOutputDimensions } from "./output-limits.ts";
...
test("generated output is resized inside the configured system bounds", async () => {
  ...
  const result = await constrainOutputDimensions(
    { image, mimeType: "image/webp", width: 120, height: 80 },
    { maxOutputWidth: 60, maxOutputHeight: 60 },
  );
  const metadata = await sharp(result.image).metadata();
