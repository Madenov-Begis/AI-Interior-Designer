...
export default async function HistoryPage() {
  ...
  return <main className="min-h-screen bg-background p-3 text-foreground sm:p-5"><div className="mx-auto max-w-[1500px] rounded-[var(--radius-lg)] border border-border bg-surface p-4 sm:p-7"><header className="flex flex-wrap items-end justify-between gap-4 border-b border-border pb-5"><div><Link href="/app" className="text-sm text-muted">← Рабочая область</Link><h1 className="mt-2 text-3xl font-black italic sm:text-4xl">История генераций</h1></div><Link href="/app/design" className="rounded-xl bg-accent px-5 py-3 text-sm font-bold text-accent-foreground">＋ Новый дизайн</Link></header><div className="mt-6"><HistoryGrid /></div></div></main>;
}
