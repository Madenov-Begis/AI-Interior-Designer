...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try { await requireAdmin(); const id = z.uuid().parse((await context.params).id); const profile = await getDb().profile.findUnique({ where: { id }, include: { plan: true, subscriptions: { orderBy: { createdAt: "desc" }, take: 10 }, _count: { select: { projects: true, generations: true } } } }); return profile ? apiSuccess(profile, requestId) : apiError("NOT_FOUND", "Пользователь не найден", requestId, 404); }
  catch (error) { return adminApiError(error, requestId, "Не удалось загрузить пользователя"); }
}
