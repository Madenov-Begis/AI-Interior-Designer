...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const dailyGenerations = await getDb().$queryRaw<Array<{ date: Date; count: bigint }>>`
      SELECT date_trunc('day', "createdAt") AS date, count(*)::bigint AS count
      FROM "Generation"
      WHERE "deletedAt" IS NULL AND "createdAt" >= ${since}
      GROUP BY 1 ORDER BY 1 ASC
    `;
    return apiSuccess({ users, activeUsers, projects, generations, generations24h, failed24h, queued, period: days, dailyGenerations: dailyGenerations.map((row) => ({ date: row.date.toISOString().slice(0, 10), count: Number(row.count) })) }, requestId);
