...
import { getAdminMediaSignedUrl } from "@/server/features/admin/service";
...
export async function GET(
  ...
  try {
    ...
    const response = apiSuccess(await getAdminMediaSignedUrl(id), requestId);
    response.headers.set("cache-control", "private, no-store");
    return response;
  } catch (error) {
    return adminApiError(error, requestId, "Не удалось открыть файл");
  }
}
