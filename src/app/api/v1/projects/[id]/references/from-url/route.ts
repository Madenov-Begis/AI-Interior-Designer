...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const results = await importReferenceUrls(user.id, projectIdSchema.parse(id), urls);
    return apiSuccess({ results }, requestId, { status: results.some((result) => !result.success) ? 207 : 201 });
  } catch (error) {
    ...
    if (error instanceof ZodError) return apiError("VALIDATION_ERROR", "Добавьте от 1 до 10 корректных URL", requestId, 400, error.flatten());
    return apiError("URL_IMPORT_FAILED", "Не удалось импортировать ссылки", requestId, 500);
  }
}
