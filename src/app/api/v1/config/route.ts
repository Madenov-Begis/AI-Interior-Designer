...
export async function GET(request: NextRequest) { return apiSuccess({ appName: "AI Interior Designer", timezone: process.env.APP_TIMEZONE ?? "Asia/Tashkent", maxSourceImageMb: 15, supportedSourceTypes: ["image/jpeg", "image/png", "image/webp"], visualPrompt: true, urlReferences: true, aiMode: (process.env.AI_PROVIDER ?? "fake") === "fake" ? "mock" : "live" }, getRequestId(request.headers)); }
