...
test("credit query invalidation follows reservation, terminal, cancellation-refund, and stale-error lifecycles", () => {
  assert.deepEqual(creditsInvalidationQueryKey("reservation"), ["auth", "me"]);
  assert.deepEqual(creditsInvalidationQueryKey("terminal"), ["auth", "me"]);
...
test("a terminal generation first observed in the list reconciles credits exactly once", () => {
  ...
  assert.deepEqual(firstObservation.queryKey, ["auth", "me"]);
  assert.deepEqual(
...
test("terminal reconciliation cancels an in-flight stale wallet read before refetching", async () => {
    ...
    queryFn: ({ signal }) => {
      ...
      if (requestNumber > 1) {
        return Promise.resolve({
          ...
          wallet: { balance: 10, generationCost: 4 },
        });
      }
      ...
      return new Promise<{
        ...
        wallet: { balance: number; generationCost: number };
      }>((_resolve, reject) => {
  ...
  assert.deepEqual(queryClient.getQueryData(["auth", "me"]), {
    user: {
      id: "user-1",
      name: "Test User",
      email: "user@example.com",
