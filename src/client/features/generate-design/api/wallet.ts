...
import { GENERATION_CREDIT_COST } from "../../../shared/config/index.ts";
import { APP_SESSION_QUERY_KEY, refreshAppSession } from "../../auth/index.ts";
    ...
    availableGenerationsText:
      wallet === null || wallet === undefined
        ...
        : `Доступно полных генераций: ${
            generationCost > 0 ? Math.floor(wallet.balance / generationCost) : 0
          }`,
    buttonLabel:
      surface === "root"
...
export function creditsInvalidationQueryKey(event: CreditsLifecycleEvent) {
  return event === "poll" ? null : APP_SESSION_QUERY_KEY;
}
...
export function reconcileTerminalCredits(
  ...
  return {
    ...
    queryKey: foundNewTerminal ? APP_SESSION_QUERY_KEY : null,
  };
}
