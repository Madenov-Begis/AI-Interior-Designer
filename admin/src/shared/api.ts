...
export async function adminApi<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!phone) throw new AdminApiError("UNAUTHORIZED", "Сессия администратора истекла");
  const response = await fetch(`${apiBase}${path}`, {
    ...init,
    headers: { Authorization: `AdminPhone ${phone}`, "Content-Type": "application/json", ...init?.headers },
  });
  const body = await response.json() as ApiEnvelope<T>;
  if (!response.ok || "error" in body) {
    const error = "error" in body ? body.error : { code: "HTTP_ERROR", message: "Не удалось выполнить запрос" };
