...
Все три согласованных пункта заключительной приёмки подтверждены: реальная загрузка → генерация → одно списание 4 кредитов → скачивание WebP; доставка email-уведомлений Sentry от сайта и админки; свежая Google-регистрация с новым профилем, записью согласия и одним начислением 10 кредитов. Доказательства и границы проверки: [заключительная приёмка](acceptance-2026-09-12.md).

Ниже сохранён исторический срез 7 сентября: упоминания о неподключённом Sentry и непроверенном полном сценарии заменены результатами отчёта выше. Остальные юридические и операционные пункты этим тестом не переоценивались.
