...
- управляемый каталог типов комнат с неизменяемым snapshot в генерации;
...
- единые системные лимиты из server environment;
- транзакционный кредитный баланс;
- локальная mock-оплата;
- отдельная Vite/Mantine админка с серверным API Next.js;
- административные API пользователей, генераций, финансов и статистики.
...
- изменять роль и статус пользователя;
...
- создавать, редактировать, включать и мягко отключать типы комнат;
...
Основной пользовательский вход выполняется через Google OAuth и Supabase Auth. Серверный PKCE callback обменивает OAuth-код на сессию и сохраняет access token в доступной клиенту cookie, а refresh token — в HttpOnly cookie приложения. Browser Supabase client не используется.

Axios читает access token через `js-cookie` и отправляет его как Bearer token. После `401` интерцептор один раз вызывает `/api/v1/auth/refresh`, получает новый access token и повторяет исходный запрос; refresh token ротируется только сервером в HttpOnly cookie. Обновление координируется между вкладками, а временная ошибка Auth не завершает сессию. Клиентские данные не считаются достаточным основанием для доступа: Route Handlers проверяют подпись access token через Supabase и статус профиля через Prisma.
...
Перед каждой новой корневой генерацией пользователь обязательно выбирает активный тип комнаты. Публичный `GET /api/v1/rooms` возвращает только `id`, неизменяемый `code` и русское `name`; серверная AI-инструкция клиенту не раскрывается. Выбор не сохраняется в `Project` и сбрасывается после успешного резервирования корневой генерации.

Каталог управляется администратором через `/rooms` и `/api/v1/admin/rooms`. Удаление является мягким отключением, повторное включение выполняется редактированием. Транзакционная блокировка не позволяет отключить последнюю активную запись.
...
- `roomTypeId` активного типа комнаты;
- допустимый aspect ratio;
- необязательный style code;
- заголовок `Idempotency-Key` длиной 16–128 символов.

Перед созданием сервер проверяет владельца проекта, наличие source, единый системный параллельный лимит и кредитный баланс. Разметка сохраняется до резервирования генерации.
...
Каждая `Generation` сохраняет неизменяемые входы: prompt, final prompt, snapshot типа комнаты (`roomTypeId`, `roomCode`, `roomName`, `roomPrompt`), style, aspect ratio, source, visual prompt, ordered references и связь с родителем. Старые записи могут иметь пустой snapshot комнаты.

Доработка создаётся от выбранного успешного результата через `parentGenerationId`. Сервер использует его чистый `resultOriginal` как единственный основной source. Доработка получает новый prompt, новую разметку и только новые референсы, явно добавленные пользователем; референсы родителя автоматически не наследуются. Тип комнаты и его серверная инструкция наследуются из snapshot родителя, даже если каталог позднее изменён или запись отключена. Retry также использует сохранённый snapshot. Результат нормализуется до точных пиксельных размеров родителя. Дочерняя генерация не изменяет родителя.
...
## 8. Системные лимиты и кредиты

### 8.1. Единые системные лимиты

Все пользователи имеют одинаковые продуктовые возможности. Тарифы, подписки,
VIP-статус и индивидуальные overrides отсутствуют. Сервер валидирует лимиты из
environment при старте:

| Переменная                 | Назначение                          | По умолчанию |
| -------------------------- | ----------------------------------- | -----------: |
| `MAX_PARALLEL_GENERATIONS` | Параллельные генерации пользователя |            1 |
| `MAX_REFERENCE_IMAGES`     | Референсы в проекте/доработке       |           10 |
| `MAX_REFERENCE_URLS`       | URL-референсы в проекте             |           10 |
| `MAX_UPLOAD_SIZE_MB`       | Размер загружаемого изображения     |           15 |
| `MAX_OUTPUT_WIDTH`         | Максимальная ширина результата      |         4096 |
| `MAX_OUTPUT_HEIGHT`        | Максимальная высота результата      |         4096 |

Дневной квоты и приоритетной очереди по классу пользователя нет. Количество
генераций дополнительно ограничивается доступным кредитным балансом.
...
- просмотр профиля, баланса и счётчиков;
...
- ограничивать параллельные генерации единым серверным лимитом;
