...
ROOVA — веб-сервис AI-визуализации дизайна интерьера. Пользователь загружает фотографию помещения, описывает желаемые изменения, при необходимости рисует визуальные указания и добавляет референсы, после чего получает фотореалистичный вариант обновлённого интерьера.
